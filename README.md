...
○
Name each table with a raw
_ prefix to differentiate between original and transformed data.
...
○
Store the transformed data using a store
_ prefix.
...
○
Join SalesOrderDetail with SalesOrderHeader on SalesOrderId and apply the following
transformations:
...
UnitPriceDiscount).
3. Write the results into a table named publish
_
orders, including:
■ All fields from SalesOrderDetail.
■ All fields from SalesOrderHeader except SalesOrderId, and rename Freight to
TotalOrderFreight.
